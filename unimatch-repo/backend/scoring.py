...
def match_score(uni: University, user: User) -> int:
    ...
    # CS fit — 28%
    score += (uni.cs_strength / 100) * 28

    # Location/climate fit — 18%
    ...
    # Financial aid fit — 10% (only weighted if the student marked aid as important)
    aid_fit = 0.7
    if user.needs_aid is True:
        aid_fit = {"High": 1.0, "Medium": 0.6, "Low": 0.25}.get(uni.aid_level, 0.5)
    score += aid_fit * 10

    # Веса выше (28+18+18+14+12+10) суммируются ровно в 100 — без произвольного
...
def why_this_university(uni: University, user: User) -> list[str]:
    ...
    if user.needs_aid is True and (uni.aid_level == "High" or uni.aid_merit):
        reasons.append(f"Финансовая помощь: {uni.aid_note}")
    if len(reasons) < 3:
